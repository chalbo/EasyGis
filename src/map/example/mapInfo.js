...
const typeColors = {
  Point: {
    fill: { color: 'red' }, stroke: { color: 'green' },
    text: { font: 'Bold 20px / 2 Arial', fill: { color: 'red' }, stroke: { color: 'green' } }
  },
  LineString: {
    fill: { color: 'yellow' }, stroke: { color: 'green' },
    text: { font: 'Bold 20px / 2 Arial', fill: { color: 'red' }, stroke: { color: 'green' } }
  },
  MultiLineString: {
    fill: { color: 'yellow' }, stroke: { color: 'green' },
  ...
  MultiPolygon: {
    fill: { color: 'rgb(123,123,123,0.1)' }, stroke: { color: 'green' },
    text: { font: 'Bold 20px / 2 Arial', fill: { color: 'red' }, stroke: { color: 'green' } }
  },
  Polygon: {
    fill: { color: 'rgb(123,123,123,0.1)' }, stroke: { color: 'green' },
...
mapInteractive.setMapLabel({ title: '这种重中之重', lonLat: [120.419354, 36.122387] }, () => {
  // console.log(111111);
  // alert(1);
});
mapInteractive.drawStar([{ count: 25, lonLat: [120.419354, 36.122387] }, { count: 25, lonLat: [101.243746, 25.109256] }], '123,123,123');
...
const handel = wfsHandle.getNoneStyleFeaturesHandle();
...
handel.on('select', (e) => {
  ...
  dic.map((layer) => { mapBase.map.removeLayer(layer) });
  // addGeoJson
  const layer1 = wfsHandle.addGeoJson(json, {
    MultiPolygon: {
      fill: { color: 'green' }, stroke: { color: 'green' },
      text: { font: 'Bold 20px / 2 Arial', fill: { color: 'red' }, stroke: { color: 'green' } }
    },
    Polygon: {
      fill: { color: 'green' }, stroke: { color: 'green' },
